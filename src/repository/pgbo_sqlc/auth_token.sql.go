...
const clearAuthTokenUserLoginByUserID = `-- name: ClearAuthTokenUserLoginByUserID :exec
UPDATE auth_token
SET
    is_login = false,
    user_login = null,
...
func (q *Queries) ClearAuthTokenUserLoginByUserID(ctx context.Context, userLogin sql.NullString) error {
	_, err := q.db.ExecContext(ctx, clearAuthTokenUserLoginByUserID, userLogin)
	return err
}

const getAuthToken = `-- name: GetAuthToken :one
SELECT t.id, t.name, t.device_id, t.device_type, t.token, t.token_expired, t.refresh_token, t.refresh_token_expired, t.is_login, t.user_login, t.ip_address, t.fcm_token, t.created_at, t.updated_at
FROM auth_token t
WHERE
    t.name = $1
    AND t.device_id = $2
...
UPDATE
    ...
    ip_address = $10,
    updated_at = (now() at time zone 'UTC')::TIMESTAMP
RETURNING auth_token.id, auth_token.name, auth_token.device_id, auth_token.device_type, auth_token.token, auth_token.token_expired, auth_token.refresh_token, auth_token.refresh_token_expired, auth_token.is_login, auth_token.user_login, auth_token.ip_address, auth_token.fcm_token, auth_token.created_at, auth_token.updated_at
`
